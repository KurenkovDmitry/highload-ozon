...
1. **Сломался сервис рекомендаций**
   - **Ситуация:** сервис рекомендаций, который формирует персонализированные предложения на основе истории просмотров, покупок и т.п., перестал работать.
   - **Решение:**
     - **Используем данные из прошлой рекомендации (если доступно):** при доступности пытаемся посмотреть последние рекомендации, которые сохранили (если поломалось именно обновление данных)
     - **Используем данные из кэша поиска:** система берёт последние популярные товары, сохранённые в кэше сервиса поиска (например, Redis), вместо того чтобы оставлять секцию рекомендаций пустой.
     - **Дополняем рандомом:** добавляем случайные товары из той же категории или популярные позиции, чтобы пользователь всё равно видел предложения.
   - **Логика возврата к нормальному режиму:**
     - **Таймеры:** система каждые 5 минут отправляет тестовый запрос (health check) к сервису рекомендаций. Если запрос проходит успешно, система переключается на использование актуальных рекомендаций.
     - **Сигналы:** сервис рекомендаций при восстановлении отправляет сообщение, уведомляя о своей готовности. После получения сигнала система немедленно возвращается к нормальному режиму.
     - Если сигнал приходит раньше очередной проверки по таймеру, система реагирует мгновенно. Если сигнала нет, восстановление происходит по результатам проверки таймера.
   - **Результат:** пользователь продолжает видеть товары в разделе рекомендаций, пусть и не полностью актуальные, что сохраняет вовлеченность и возможность покупок.

Схематичное отображение этого сценария можно посмотреть по ссылке: [https://draft.io/tsddm73tu8pu3nrgk4gvhrhukqkrvzv2atazs6qr2yhf](https://draft.io/tsddm73tu8pu3nrgk4gvhrhukqkrvzv2atazs6qr2yhf)

Либо в виде изображения:

![img.png](img%2F9%2Fimg.png)

> **Комментарий:**  
> На архитектурной схеме с реализацией сценария Graceful Degradation стрелками влево (не ведущие в какие-либо блоки) схематично показано, что это конечный блок и он возвращает результат.

2. **Отказ сервиса оплаты**
   - **Ситуация:** сервис обработки платежей временно недоступен, что может прервать процесс оформления заказа.
   - **Решение:**
     - **Сообщение пользователю:** система сообщает пользователю, что всё в порядке, и оплата будет списана после проверки и сборки товара. Это создаёт иллюзию бесперебойной работы и сохраняет спокойствие пользователя. 
     - **Сохранение в стек:** заказ сохраняется в специальном стеке для последующей обработки, пока система пытается завершить оплату в фоновом режиме.
     - **Обработка при сбоях:**
       - Если сервис оплаты не восстанавливается и фоновая обработка не удаётся, пользователю выводится сообщение: "Оплата временно недоступна, пожалуйста, попробуйте позже".
       - Корзина при этом не очищается, чтобы пользователь не потерял выбранные товары.
     - **Стимулы для удержания:**
       - Пользователю предлагаются скидки, промокоды и персонализированные рекомендации по другим товарам, чтобы поддерживать интерес и лояльность.
   - **Логика возврата к нормальному режиму:**
     - **Таймеры:** каждые 10 минут система пытается обработать отложенные заказы, отправляя тестовый запрос к сервису оплаты. При успешной обработке тестового платежа система возобновляет нормальную работу.
     - **Сигналы:** сервис оплаты при восстановлении отправляет уведомление. После получения сигнала система сразу начинает обрабатывать платежи в нормальном режиме.
     - Если сигнал о восстановлении приходит раньше таймера, система мгновенно переключается. Если сигнала нет, восстановление происходит по таймеру после успешного теста.
   - **Результат:** пользователь не теряет заказ и может завершить покупку позже, когда сервис оплаты восстановится. Магазин минимизирует потери благодаря сохранению заказа и удержанию клиента с помощью дополнительных стимулов.

Схематичное отображение этого сценария можно посмотреть по ссылке: [https://draft.io/bwvqkw3gwkcsphqbp6f2mh7y5gvtcg3y4ndh6ttrtxyh](https://draft.io/bwvqkw3gwkcsphqbp6f2mh7y5gvtcg3y4ndh6ttrtxyh)

Либо в виде изображения:

![img_1.png](img%2F9%2Fimg_1.png)
...
С учетом резерва (1+1 на ДЦ) и 6 дата-центров:
- **Всего Nginx-серверов**: 5 * 2 * 6 = **60 серверов**.
...
#### Для фронтенда
- **8 Redis** (кэширование для фронтенда):
    - **Логическое количество**: 8 Redis.
    - **Физическое количество**: для хайлоада предположим кластер с репликацией (3 master + 3 replica на кластер) и распределением по 6 дата-центрам. Если объединить в один кластер, то:
        - **5 машин** (3 master + 2 replica) на дата-центр * 6 ДЦ = **30 машин**.
        - **Конфигурация**: 16 ядер, 64 ГБ RAM, 1TB SSD.

#### Для бэкенда
- **5 PostgreSQL**:
    - **Логическое количество**: 5 баз.
    - **Физическое количество**: для хайлоада каждый PostgreSQL-кластер должен иметь репликацию (1 master + 2 replica) на основном и 1 + 1 на других.
        - 5 кластеров * (1 master + 1 replica) = 10 машин на ДЦ; 5 кластер * (1 master + 2 replica) = 15 машин на ДЦ.
        - С учетом 6 ДЦ: 15 * 1 + 10 * 5 = **65 машин**.
        - **Конфигурация**: 32 ядра, 128 ГБ RAM, 2TB SSD.

- **1 ClickHouse** (аналитика):
    - **Логическое количество**: 1 кластер для аналитики.
    - **Физическое количество**: для хайлоада нужен кластер с репликацией и шардированием (4 узла на ДЦ). С учетом 6 ДЦ:
        - 4 узла * 6 ДЦ = **24 машин**.
        - **Конфигурация**: 16 ядер, 64 ГБ RAM, 2TB SSD.

- **3 Redis** (кэширование для бэкенда):
    - **Логическое количество**: 3 Redis.
    - **Физическое количество**: если объединить в один кластер, то:
        - 5 машин (3 master + 2 replica) на ДЦ * 6 ДЦ = **30 машин**.
        - **Конфигурация**: 16 ядер, 64 ГБ RAM, 1TB SSD.

- **1 Elasticsearch** (поиск по товарам):
    - **Логическое количество**: 1 кластер для поиска.
    - **Физическое количество**: для хайлоада нужен кластер с репликацией (3 узла на ДЦ). С учетом 6 ДЦ:
        - 3 узла * 6 ДЦ = **18 машин**.
        - **Конфигурация**: 16 ядер, 64 ГБ RAM, 2TB SSD.

- **3 Amazon S3** (хранилище для изображений, файлов):
    - **Логическое количество**: 3 S3.
    - **Физическое количество**: Amazon S3 — это объектное хранилище, и физически оно не требует отдельных серверов, но для хайлоада можно использовать 3 логических бакета в одном S3-кластере. Для расчета предположим эквивалент 3 машин на ДЦ для обработки запросов:
        - 3 машины * 6 ДЦ = **18 машин**.
        - **Конфигурация**: 16 ядер, 64 ГБ RAM, 2TB SSD (для шлюзов или кэширования).

- **7 Kafka** (очереди сообщений, аналитика):
    - **Логическое количество**: 7 кластеров, возможно, для разных потоков событий.
    - **Физическое количество**: для хайлоада объединим в один кластер с большим числом брокеров (7 брокеров на ДЦ). С учетом 6 ДЦ:
        - 7 брокеров * 6 ДЦ = **42 машины**.
        - **Конфигурация**: 32 ядра, 128 ГБ RAM, 2TB SSD.
...
- **2 Grafana** (визуализация метрик):
    ...
    - **Физическое количество**: для хайлоада достаточно 2 узла на ДЦ с резервированием. С учетом 6 ДЦ:
        - 2 узла * 6 ДЦ = **12 машин**.
        - **Конфигурация**: 8 ядер, 16 ГБ RAM, 500GB SSD.

- **11 Prometheus** (мониторинг):
    - **Логическое количество**: 11 инстансов.
    - **Физическое количество**: для хайлоада объединим в кластер с 5 узлами на ДЦ (для масштабируемости). С учетом 6 ДЦ:
...
- **Итого**: 29,9 + 0,7 = **30,6 млн. руб. в месяц**

### Стоимость покупки серверов

#### Kubernetes-ноды
- **Конфигурация**: 64 ядра, 256 ГБ RAM, 4TB NVMe
- **Примерная стоимость**: 900,000 руб.
- **Количество**: 69
- **Общая стоимость**: 69 * 900,000 = **62,100,000 руб.**

#### Nginx
- **Конфигурация**: 16 ядер, 32 ГБ RAM
- **Примерная стоимость**: 270,000 руб.
- **Количество**: 60
- **Общая стоимость**: 60 * 270,000 = **16,200,000 руб.**

#### Kafka
- **Конфигурация**: 32 ядра, 128 ГБ RAM, 2TB SSD
- **Примерная стоимость**: 540,000 руб.
- **Количество**: 42
- **Общая стоимость**: 42 * 540,000 = **22,680,000 руб.**

#### PostgreSQL
- **Конфигурация**: 32 ядра, 128 ГБ RAM, 2TB SSD
- **Примерная стоимость**: 540,000 руб.
- **Количество**: 65
- **Общая стоимость**: 65 * 540,000 = **35,100,000 руб.**

#### Redis (фронтенд)
- **Конфигурация**: 16 ядер, 64 ГБ RAM, 1TB SSD
- **Примерная стоимость**: 360,000 руб.
- **Количество**: 30
- **Общая стоимость**: 30 * 360,000 = **10,800,000 руб.**
...
#### ClickHouse
- **Конфигурация**: 16 ядер, 64 ГБ RAM, 2TB SSD
- **Примерная стоимость**: 405,000 руб.
- **Количество**: 24
- **Общая стоимость**: 24 * 405,000 = **9,720,000 руб.**

#### Elasticsearch
- **Конфигурация**: 16 ядер, 64 ГБ RAM, 2TB SSD
- **Примерная стоимость**: 405,000 руб.
- **Количество**: 18
- **Общая стоимость**: 18 * 405,000 = **7,290,000 руб.**

#### Amazon S3 (шлюзы)
- **Конфигурация**:  counted as 16 ядер, 64 ГБ RAM, 2TB SSD
- **Примерная стоимость**: 405,000 руб.
- **Количество**: 18
- **Общая стоимость**: 18 * 405,000 = **7,290,000 руб.**
...
- **Примерная стоимость**: 360,000 руб.
- **Количество**: 18
- **Общая стоимость**: 18 * 360,000 = **6,480,000 руб.**

#### Grafana
- **Конфигурация**: 8 ядер, 16 ГБ RAM, 500GB SSD
- **Примерная стоимость**: 180,000 руб.
- **Количество**: 12
- **Общая стоимость**: 12 * 180,000 = **2,160,000 руб.**
...
62,100,000 + 16,200,000 + 22,680,000 + 35,100,000 + 10,800,000 + 10,800,000 + 9,720,000 + 7,290,000 + 7,290,000 + 7,290,000 + 6,480,000 + 2,160,000 + 10,800,000 = **192,429,000 руб.**

### Учет амортизации и операционных расходов

#### Амортизация
Сервера будут заменены через 5 лет (60 месяцев). Ежемесячная амортизация:
- **Амортизация**: 192,429,000 / 60 ≈ **3,207,150 руб./мес.**

#### Электроэнергия
Оценим потребление электроэнергии:
- **Общее количество серверов**: 69 + 60 + 42 + 65 + 30 + 30 + 24 + 18 + 18 + 18 + 18 + 12 + 30 = **406 серверов**
- **Среднее потребление на сервер**: 500 Вт (примерно для серверов с 16–64 ядрами)
- **Общее потребление**: 406 * 500 = 203,000 Вт = 203 кВт
- **Ежемесячное потребление**: 203 кВт * 24 ч * 30 дней = 146,160 кВтч
- **Стоимость электроэнергии**: 5 руб./кВтч
- **Ежемесячная стоимость**: 146,160 * 5 = **730,800 руб./мес.**

#### Другие операционные расходы
Учтем обслуживание, охлаждение и аренду помещения как 20% от стоимости электроэнергии:
- **Другие расходы**: 730,800 * 0.2 = **146,160 руб./мес.**

#### Итоговые ежемесячные расходы
- Амортизация: 3,207,150 руб./мес.
- Электроэнергия: 730,800 руб./мес.
- Другие расходы: 146,160 руб./мес.
- **Всего**: 3,207,150 + 730,800 + 146,160 = **4,084,110 руб./мес.**

### Расчет срока окупаемости

#### Ежемесячная экономия
- **Экономия**: 30,600,000 - 4,084,110 = **26,515,890 руб./мес.**

#### Срок окупаемости
- **Срок**: 192,429,000 / 26,515,890 ≈ **7.26 месяцев**
