...
|------------------|---------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------|--------------|------------------|--------------------|----------------------------------------------------|
| `Buyer`          | Хранит основные данные покупателей: имя, контактные данные, метрики для рекомендаций. | `id` (PK), `email` (уникальный), `phone` (уникальный), `create_at`, `changed_at`, `date_of_last_visit` | `recommendation_data` в JSONB для хранения просмотров и покупок без JOIN. | PostgreSQL   | По `id`          | Репликация         | SQLAlchemy (Python), Hibernate (Java), Redis (кэш) |
...
| `gender`         | Содержит пол пользователя (мужской, женский, другой).                                 | `id` (PK)`                                                                                             | Нет                                                                       | PostgreSQL   | По `id`          | Репликация         | SQLAlchemy, Hibernate                              |
...
- **СУБД:** PostgreSQL для сложных транзакций, Redis для сессий из-за высокой скорости доступа.
...
| **Таблица**    | **Описание**                                       | **Индексы**                                          | **Денормализация**                                       | **СУБД**       | **Шардирование** | **Резервирование** | **Клиентские библиотеки / Интеграции** |
|----------------|----------------------------------------------------|------------------------------------------------------|----------------------------------------------------------|----------------|------------------|--------------------|----------------------------------------|
| `order`        | Заказы: товары, адрес, статус оплаты.              | `id` (PK), `buyer_id`, `created_at`, `delivery_date` | `products` и `address_data` в JSONB для уменьшения JOIN. | PostgreSQL     | По `buyer_id`    | Репликация         | SQLAlchemy, Hibernate, Kafka (очереди) |
| `order-status` | Статусы заказов: текущий статус и время изменения. | `order_id` (PK)                                      | Нет                                                      | PostgreSQL     | По `order_id`    | Репликация         | SQLAlchemy, Hibernate                  |
| `address`      | Адреса доставки: страна, город, улица.             | `address_id` (PK)                                    | Нет                                                      | PostgreSQL     | По `address_id`  | Репликация         | SQLAlchemy, Hibernate                  |
| `paid-method`  | Способы оплаты: название, параметры.               | `id` (PK)                                            | `parameters` в JSONB для гибкости.                       | PostgreSQL     | По `id`          | Репликация         | SQLAlchemy, Hibernate                  |
| `status`       | Содержит статусы заказов.                          | `id` (PK)`                                           | Нет                                                      | PostgreSQL     | По `id`          | Репликация         | SQLAlchemy, Hibernate                  |
...
- **Выбор СУБД:** PostgreSQL для транзакций и сложных запросов, Elasticsearch для поиска, Redis для сессий.
