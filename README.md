...
2. **Рекомендательная система**
   - Динамическая лента товаров, формируемая на основе популярных категорий, трендов или простых алгоритмов (например, "Хиты продаж", "Рекомендуем вам"). Заложить учет в дальнейшее увеличени точности - учет отзывов, оценок, количества просмотров, частота наведения, время просмотра страницы товара и т.д.
...
Идентификатор (16 байт), название (300 байт - 150 символов), характеристики (2 килобайта), описание (2,5 килобайта), несколько фотографий (7 * 75 = 525 килобайт), цена (4 байта), список идентификаторов отзывов (32 килобайта), количество (4 байта), таким образом в среднем занимаемый объем ≈ 16 + 300 + 2 * 1024 + 2,5 * 1024 + 525 + 4 + 32 * 1024 + 4 = 38225 байт 
...
Начнем с пользователей, которых 58,18 млн., тогда объем для их хранения примерно 0,00001749 * 58,18 * 10^(6) ≈ 1017,57 гигабайт
...
А вот уже по 6 таблице каждый продавец выставляет пол товара в сутки (в среднем), а если ещё взять под расчет, то что по таблице 3 продавцов 575 тыс., то за месяц товары займут 575 * 10^(3) * 0,5 * 30 * 0,00003560 = 307,05 гигабайт.
...
Нужно хранить идентификатор (16 байт (примерная оценка опирается от uuid)), ФИО (60 байт - 30 символов), пол (1 байт - идентификатор (м, ж, не известно)), почту (40 байт - 20 символов), аватарку (18 килобайт), телефон (24 байта - 12 символов), пароль (16 байт - 8 символов), адрес доставки (90 байт - 45 символов), дата рождения (8 байт), дата регистрации (8 байт), активность (1 байт), идентификаторы товаров (16 * 546 = 8736 байт), таким образом в среднем занимаемый объем ≈ 16 + 60 + 1 + 40 + 18 * 1024 + 24 + 16 + 90 + 8 + 8 + 1 + 8736 = 27432 байт ≈ 26,79 килобайтов

Начнем с покупателей, которых 575 тыс., тогда объем для их хранения примерно 0,00002555 * 575 * 10^(3) ≈ 14,69 гигабайт
