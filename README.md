...
| **PgBouncer**     | Пулинг соединений PostgreSQL      | Уменьшает нагрузку на БД за счет пулинга подключений. Критичен для высоконагруженных систем (10k+ RPS). Поддержка транзакционного/сессионного режимов.                                                   |
...
| **GraphQL**               | Гибкие запросы данных (каталог товаров)                   | Позволяет клиентам запрашивать только нужные данные. Снижает нагрузку на бэкенд. Используется Apollo Server для Java.                                                            |
...
| **OWASP ZAP**           | Динамический анализ       | Проверка веб-приложения на уязвимости во время выполнения.                                 |
