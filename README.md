...
|-------------------|-------------------------------------------------------------------------------------------------------|--------------------------------|---------------------------------------------------------------------|--------------|------------------|--------------------|----------------------------------------------------|
| `session`         | Данные сессий пользователей для аутентификации и аналитики.                                           | `session_id` (PK), `buyer_id`  | Нет                                                                 | Redis        | По `buyer_id`    | Репликация         | Redis-py (Python), Jedis (Java)                    |
| `cash-buyer`      | Кэш данных покупателя: имя, телефон, изображение, город, страна. Связан с сессией через `session_id`. | `session_id` (PK)              | Данные дублируются из `Buyer` для быстрого доступа по `session_id`. | Redis        | По `session_id`  | Репликация         | Redis-py (Python), Jedis (Java)                    |
| `cash-cart`       | Кэш корзины: список товаров, общая сумма. Связан с сессией через `session_id`.                        | `session_id` (PK)              | `products` в JSONB для хранения списка товаров без JOIN.            | Redis        | По `session_id`  | Репликация         | Redis-py (Python), Jedis (Java)                    |
| `cash-categories` | Кэш категорий продуктов: структура категорий и подкатегорий. Связан с сессией через `session_id`.     | `session_id` (PK)              | `categories` в JSONB для хранения вложенных данных категорий.       | Redis        | По `session_id`  | Репликация         | Redis-py (Python), Jedis (Java)                    |
...
| **Таблица**      | **Описание**                                                                          | **Индексы**                                                                                            | **Денормализация**                                                                                            | **СУБД**     | **Шардирование** | **Резервирование** | **Клиентские библиотеки / Интеграции**             |
|------------------|---------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------|--------------|------------------|--------------------|----------------------------------------------------|
| `Buyer`          | Хранит основные данные покупателей: имя, контактные данные, метрики для рекомендаций. | `id` (PK), `email` (уникальный), `phone` (уникальный), `create_at`, `changed_at`, `date_of_last_visit` | `recommendation_data` в JSONB для хранения просмотров и покупок без JOIN; `gender` в JSONB для опознания пола | PostgreSQL   | По `id`          | Репликация         | SQLAlchemy (Python), Hibernate (Java), Redis (кэш) |
| `Buyer-passport` | Паспортные данные покупателей для идентификации.                                      | `buyer_id` (PK)                                                                                        | Нет                                                                                                           | PostgreSQL   | По `buyer_id`    | Репликация         | SQLAlchemy, Hibernate                              |
| `Buyer-contacts` | Дополнительные контакты покупателей для поиска и рекламы.                             | `buyer_id` (PK), `phone`                                                                               | Нет                                                                                                           | PostgreSQL   | По `buyer_id`    | Репликация         | SQLAlchemy, Hibernate                              |
| `cart`           | Корзина покупателя: товары, адрес доставки.                                           | `buyer_id` (PK)                                                                                        | `products` в JSONB для хранения списка товаров без JOIN.                                                      | PostgreSQL   | По `buyer_id`    | Репликация         | SQLAlchemy, Hibernate, Redis (кэш)                 |
...
| **Таблица**                | **Описание**                                                 | **Индексы**                                                 | **Денормализация**                                                             | **СУБД**       | **Шардирование**  | **Резервирование** | **Клиентские библиотеки / Интеграции**      |
|----------------------------|--------------------------------------------------------------|-------------------------------------------------------------|--------------------------------------------------------------------------------|----------------|-------------------|--------------------|---------------------------------------------|
| `product`                  | Основные данные о продуктах: название, цена, характеристики. | `id` (PK), `seller_id`, `title`, `created_at`, `changed_at` | `characteristics`, `breadcrumbs`, `img` в JSONB для быстрого доступа без JOIN. | PostgreSQL     | По `id`           | Репликация         | SQLAlchemy, Hibernate                       |
| `product-filter`           | Данные для фильтрации продуктов: цена, вес, категории.       | `product_id` (PK)                                           | Дублирует данные из `product` и `product-search` для ускорения фильтрации.     | PostgreSQL     | По `product_id`   | Репликация         | SQLAlchemy, Hibernate                       |
| `product-reviews`          | Количество отзывов по продуктам.                             | `product_id` (PK)                                           | Нет                                                                            | PostgreSQL     | По `product_id`   | Репликация         | SQLAlchemy, Hibernate                       |
| `rating-product`           | Рейтинг продуктов: количество оценок и их сумма.             | `product_id` (PK)                                           | Нет                                                                            | PostgreSQL     | По `product_id`   | Репликация         | SQLAlchemy, Hibernate                       |
| `recomendation`            | Данные для рекомендаций: посещения, покупки, рейтинги.       | `product_id` (PK)                                           | Денормализована для быстрого доступа к данным рекомендаций.                    | PostgreSQL     | По `product_id`   | Репликация         | SQLAlchemy, Hibernate                       |
| `product-categories`       | Категории продуктов и подкатегории.                          | `id` (PK), `title`                                          | `subcategories` в JSONB для хранения вложенных данных.                         | PostgreSQL     | По `id`           | Репликация         | SQLAlchemy, Hibernate                       |
| `seller-rating`            | Рейтинг продавцов: сумма и количество оценок.                | `seller_id` (PK)                                            | Нет                                                                            | PostgreSQL     | По `seller_id`    | Репликация         | SQLAlchemy, Hibernate                       |

#### БД для поиска (Elasticsearch)
| **Таблица**                | **Описание**                                                 | **Индексы**                                                 | **Денормализация**                                                             | **СУБД**       | **Шардирование**  | **Резервирование** | **Клиентские библиотеки / Интеграции**      |
|----------------------------|--------------------------------------------------------------|-------------------------------------------------------------|--------------------------------------------------------------------------------|----------------|-------------------|--------------------|---------------------------------------------|
| `product-search`           | Данные для полнотекстового поиска: название, метрики.        | `product_id` (PK), `title`                                  | Денормализована для быстрого поиска, дублирует данные из `product`.            | Elasticsearch  | По `product_id`   | Репликация         | Elasticsearch-Py (Python), Java REST Client |
...
| **Таблица**    | **Описание**                                  | **Индексы**                                         | **Денормализация**                                  | **СУБД**      | **Шардирование**  | **Резервирование** | **Клиентские библиотеки / Интеграции**      |
|----------------|-----------------------------------------------|-----------------------------------------------------|-----------------------------------------------------|---------------|-------------------|--------------------|---------------------------------------------|
| `review`       | Отзывы о продуктах: текст, автор, медиа.      | `id` (PK), `product_id`, `author_id`, `added_at`    | `img` в JSONB для хранения медиа без JOIN.          | PostgreSQL    | По `product_id`   | Репликация         | SQLAlchemy, Hibernate, Kafka (очереди)      |
| `review-like`  | Лайки и дизлайки отзывов.                     | `review_id` (PK)                                    | Нет                                                 | PostgreSQL    | По `review_id`    | Репликация         | SQLAlchemy, Hibernate, Kafka (очереди)      |
...
| **Таблица**                 | **Описание**                                                | **Индексы**                                   | **Денормализация** | **СУБД**       | **Шардирование**    | **Резервирование** | **Клиентские библиотеки / Интеграции** |
|-----------------------------|-------------------------------------------------------------|-----------------------------------------------|--------------------|----------------|---------------------|--------------------|----------------------------------------|
| `seller-data`               | Данные продавцов: ИНН, банковские реквизиты.                | `buyer_id` (PK)                               | Нет                | PostgreSQL     | По `buyer_id`       | Репликация         | SQLAlchemy, Hibernate                  |
| `seller-rating-buyer`       | Рейтинг продавцов от покупателей.                           | `buyer_id` (PK)                               | Нет                | PostgreSQL     | По `buyer_id`       | Репликация         | SQLAlchemy, Hibernate                  |
| `seller-products-analytics` | Аналитика продуктов продавца: посещения, покупки, рейтинги. | `id` (PK), `buyer_id`, `product_id`           | Нет                | PostgreSQL     | По `buyer_id`       | Репликация         | SQLAlchemy, Hibernate                  |
| `seller-product-main-data`  | Основные данные продуктов продавца: цена, количество.       | `selproana_id` (PK), `buyer_id`, `product_id` | Нет                | PostgreSQL     | По `buyer_id`       | Репликация         | SQLAlchemy, Hibernate                  |
...
- **Шардирование:** По `seller_id` или `buyer_id` для распределения нагрузки.
- **Резервирование:** Репликация для отказоустойчивости.

### 5. Заказы и корзины
| **Таблица**    | **Описание**                                       | **Индексы**                                          | **Денормализация**                                                                                                      | **СУБД**       | **Шардирование** | **Резервирование** | **Клиентские библиотеки / Интеграции** |
|----------------|----------------------------------------------------|------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------|----------------|------------------|--------------------|----------------------------------------|
| `order`        | Заказы: товары, адрес, статус оплаты.              | `id` (PK), `buyer_id`, `created_at`, `delivery_date` | `products`, `paid_method` (внутри него `parameters` в JSONB для гибкости) и `address_data` в JSONB для уменьшения JOIN. | PostgreSQL     | По `buyer_id`    | Репликация         | SQLAlchemy, Hibernate, Kafka (очереди) |
| `order-status` | Статусы заказов: текущий статус и время изменения. | `order_id` (PK)                                      | `status` в JSONB для определения статуса заказа                                                                         | PostgreSQL     | По `order_id`    | Репликация         | SQLAlchemy, Hibernate                  |
| `address`      | Адреса доставки: страна, город, улица.             | `address_id` (PK)                                    | Нет                                                                                                                     | PostgreSQL     | По `address_id`  | Репликация         | SQLAlchemy, Hibernate                  |
...
- **Выбор СУБД:** PostgreSQL для транзакций и сложных запросов, Elasticsearch для поиска, Redis для сессий и кешей.
...
- **Применение:**
   ...
   - **Пользовательские данные (PostgreSQL)**: Данные пользователей (`Buyer`, `cart`) шардированы по регионам (например, данные российских пользователей хранятся в российских ЦОДах). Запросы на чтение обслуживаются локальными репликами, а запись идет на мастер в основном ЦОДе.
...
- **Инструменты:**
   - GeoDNS-серверы (например, Cloudflare, Route 53).
...
Схема резервного копирования должна учитывать географическое распределение данных и обеспечивать отказоустойчивость как на глобальном, так и на локальном уровнях.

#### Принципы
- **Регулярность**: Копии создаются ежедневно или чаще.
- **Полнота**: Включают все данные для восстановления.
- **Географическое распределение**: Копии хранятся в разных регионах.
...
- **Сессии и кэши (Redis)**:
   ...
   - Хранение: Копии реплицируются в другой ЦОД или облако (например, AWS S3).
- **Пользовательские данные, продукты, отзывы, продавцы, заказы (PostgreSQL)**:
   - Полные копии через `pg_basebackup` еженедельно.
   - Инкрементные копии через WAL ежедневно, отправляются в центральное хранилище (S3).
   - Хранение: Копии в ЦОДе Москвы и облаке.
- **Продукты (Elasticsearch)**:
   - Полные снимки еженедельно, инкрементные — ежедневно.
   - Хранение: Репозиторий в облаке (S3) или другом ЦОДе.
...
- Глобально: Копии распределяются между ЦОДами для защиты от региональных сбоев.
- Локально: Копии используются для быстрого восстановления внутри ЦОДа.

### Итоговая архитектура
- **Балансировка запросов**:
   - **Глобально**: GeoDNS и BGP Anycast направляют трафик к ближайшему ЦОДу, данные реплицируются или шардируются по регионам.
   - **Локально**: HAProxy распределяют запросы между шардами и репликами внутри ЦОДа.
- **Мультиплексирование подключений**: PgBouncer и клиентские библиотеки оптимизируют подключения на обоих уровнях.
- **Резервное копирование**:
   - **Глобально**: Копии хранятся в разных ЦОДах и облаке.
   - **Локально**: Локальные копии обеспечивают быстрое восстановление.

> ![IMPORTANT]
> Важно уточнить структуру - есть центральный ЦОД с хранилищем в Москве. Запросы на чтение отправляются в ближайшие ЦОДы, а запись дублируется в ближайший и в центральный, который раз в определенное время рассылает свои данные на другие ЦОДы. Это нужно для поддержания целостности (например товар сначала добавляется в одном ЦОДе и в центральном, а затем рассылается на другие).
