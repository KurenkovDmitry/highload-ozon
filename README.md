...
| `recomendation`            | Данные для рекомендаций: посещения, покупки, рейтинги.       | `product_id` (PK)                                           | Денормализована для быстрого доступа к данным рекомендаций.                    | PostgreSQL     | По `product_id`  | Репликация + WAL + pg_basebackup  | SQLAlchemy, Hibernate                       |
...
| `seller-product-main-data`  | Основные данные продуктов продавца: цена, количество.       | `selproana_id` (PK), `buyer_id`, `product_id` | Нет                | PostgreSQL     | По `buyer_id`       | Репликация + WAL + pg_basebackup  | SQLAlchemy, Hibernate                  |
